...
public class EquipmentActionTest  extends ZoneAndPlayerTestImpl {
	...
	/**
	 * Test adding 'mithril clasp' to keyring.
	 */
	@Test
	public void testMithrilClaspToKeyring() {
		
		// Create mithril clasp
		Item clasp = SingletonRepository.getEntityManager().getItem("mithril clasp");
		
		// Test if clasp is compatible with "keying" slots
		assertTrue("Equipping player with 'mythril clasp' in keyring should be successfull.", clasp.getPossibleSlots().contains("keyring"));
	}
	
	/**
	 * Test adding stacked items to a keyring
	 */
	@Test
	public void testStackabletoKeyring()
	{
		// setting up testing zone
		StendhalRPZone localzone = new StendhalRPZone("testzone", 20, 20);
        SingletonRepository.getRPWorld().addRPZone(localzone);
        
        // creating player, keyring and lucky charms
        final Player player = PlayerTestHelper.createPlayer("bob");
        ...
        // equipping them to the bag and then setting players keyring feature to true
        player.equip("bag", keyring);
        player.equip("bag", charm);
        player.setFeature("keyring", true);
        
        // checking the keyring and lucky charms are equipped
        assertTrue(player.isEquipped("keyring"));
        assertTrue(player.isEquipped("lucky charm"));
        
        // adding player to testing zone
        localzone.add(player);
        
        // creating new RPAction and setting it to equip
        RPAction equip = new RPAction();
        equip.put("type", "equip");
        
        // creating the sourcepath
        List<String> srcPath = Arrays.asList(Integer.toString(player.getID().getObjectID()), "bag", Integer.toString(charm.getID().getObjectID()));
        equip.put(EquipActionConsts.SOURCE_PATH, srcPath);
        
        // creating the targetpath
        List<String> path = Arrays.asList(Integer.toString(player.getID().getObjectID()), "keyring");
        equip.put(Actions.TARGET_PATH, path);
        
        
        // doing the equip action on the player
        EquipAction action = new EquipAction();
        action.onAction(player, equip);
        
        // checking that the charms are not on the keyring
        assertFalse("Equipping stacked items to keyring should be unsuccesful.",player.getSlot("keyring").getFirst() == charm);
	}
}
