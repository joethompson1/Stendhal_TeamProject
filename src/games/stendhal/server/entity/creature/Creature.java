...
public class Creature extends NPC {
	...
	/**
	 * Returns the nearest npc (all RPEntities except Players), which is reachable.
	 *
	 * @param range
	 *            attack radius
	 * @return chosen npc or null if no npc was found.
	 */
	public RPEntity getNearestRPEntity(final double range, Player owner) {
		// create list of NPCs
		final List<NPC> npcList = getZone().getNPCList();
		if (npcList.isEmpty()) {
			return null;
		}
		
		// create list of Players
		List<Player> playerList = getZone().getPlayers();
		playerList.remove(owner);
		if (npcList.isEmpty()) {
			return null;
		}

		// calculate the distance of all possible NPC's
		final Map<RPEntity, Double> distances = new HashMap<RPEntity, Double>();
		for (final RPEntity npc : npcList) {
			if (npc == this) {
				continue;
			}

			if (npc.isInvisibleToCreatures()) {
				continue;
			}

			final double squaredDistance = this.squaredDistance(npc);
			if (squaredDistance <= (range * range)) {
				distances.put(npc, squaredDistance);
			}
		}
		for (final RPEntity player : playerList) {
			if (player == this) {
				continue;
			}
			...
			final double squaredDistance = this.squaredDistance(player);
			if (squaredDistance <= (range * range)) {
				distances.put(player, squaredDistance);
			}
		}

		// now choose the nearest npc for which there is a path, or is
		// attackable otherwise
		RPEntity chosen = null;
		while ((chosen == null) && !distances.isEmpty()) {
			double shortestDistance = Double.MAX_VALUE;
			for (final Map.Entry<RPEntity, Double> npc : distances.entrySet()) {
				final double distance = npc.getValue();
				if (distance < shortestDistance) {
					chosen = npc.getKey();
					shortestDistance = distance;
				}
...
//				if ((path == null) || path.isEmpty() && !strategy.canAttackNow(this, chosen)) {
		...
		// return the chosen npc or null if we could not find one in reach
		return chosen;
	}
	...
	public boolean isNpcNear(final double range) {
		final int x = getX();
		final int y = getY();
	
		List<NPC> npcList = getZone().getNPCList();
		if (npcList.isEmpty()) {
			return false;
		}
		...
		for (final RPEntity npc : npcList) {
			...
			if (npc.getZone() == getZone()) {
				final int fx = npc.getX();
				final int fy = npc.getY();
